...
ListNode *ListNode::remove(ListNode *head, int position)//removing a node at a given position within an already existing linked list 
{
    int counter = 0;
    ListNode *currNode = head;
    ListNode *prevNode = head;
    while (counter < position)//traversing the list until the position of the node that is going to be removed 
    {
        prevNode = currNode;
        currNode = currNode->getNext();
        counter ++;
